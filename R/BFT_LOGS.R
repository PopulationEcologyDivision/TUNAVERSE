...
BFT_LOGS <- function(tblList){
  ...
  SLIP_SPC_STD_INFO_wide <- tblList$SLIP_SPC_STD_INFO %>%
    ...
    dplyr::mutate(
      WEIGHT_LBS = dplyr::if_else(UNIT_OF_MEASURE_ID == 10, WEIGHT * 2.20462, WEIGHT), #kg to lbs
      WEIGHT_LBS = dplyr::if_else(SSF_SPECIES_CODE == 254 & SSF_LANDED_FORM_CODE == 1, WEIGHT_LBS / 1.25, WEIGHT_LBS) #landed_form of 1 gets divided by 1.25
    ) %>% 
  ...
  LOG <- merge(tblList$MON_DOCS[,c("MON_DOC_ID", "MON_DOC_DEFN_ID", "FV_NAFO_UNIT_AREA_ID", "FV_GEAR_CODE")], 
               tblList$LOG_EFRT_STD_INFO[,c("MON_DOC_ID", "LOG_EFRT_STD_INFO_ID", "ENT_LATITUDE", "DET_LATITUDE", "ENT_LONGITUDE", "DET_LONGITUDE", "DET_NAFO_UNIT_AREA_ID", "FV_NAFO_UNIT_AREA_ID", "FV_FISHED_DATETIME")], by = "MON_DOC_ID")
  ...
  LOG <- LOG %>%
    dplyr::mutate(
      ...
      gear_code = dplyr::coalesce(FV_GEAR_CODE, Gear)
    ) %>%
    dplyr::select(-ENT_LATITUDE, -DET_LATITUDE, -ENT_LONGITUDE, -DET_LONGITUDE, -DET_NAFO_UNIT_AREA_ID, -LE_FV_NAFO_UNIT_AREA_ID, -MD_FV_NAFO_UNIT_AREA_ID, -FV_GEAR_CODE, -Gear)
