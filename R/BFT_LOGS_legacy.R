...
#' @importFrom dplyr %>%
#' @export
BFT_LOGS_legacy = function(dsn = "PTRAN 64bit", uid=NULL,pwd=NULL) {
  PTRAN = RODBC::odbcConnect(dsn = dsn,
                               uid = uid, 
                               pwd = pwd)
  results = data.table(RODBC::sqlQuery(PTRAN, paste( 
  "SELECT",
  "trip.mon_doc_id,",
  ...
  "community_code,",
  "sum(decode(ssf_species_code || ssf_species_size_code || ssf_landed_form_code || unit_of_measure_id, 2549320, weight,2549310, weight * 2.2046, 2549120,weight/1.25,2549110, weight * 2.2046/1.25,null)) BFT_dressed_weight_lbs_trip,", 	##  bluefin dressed weight lbs
